...
module.exports = React.createClass({
  ...
  handlePlayerChange: function(event) {
    var assistedById =
      this.state.assistedById == event.target.value
        ? null
        : this.state.assistedById;

    var playerId = event.target.value == "ingen" ? null : event.target.value;
  ...
  handleAssistChange: function(event) {
    var assistedById =
      event.target.value == "ingen" ? null : event.target.value;
  ...
  deleteEvent: function(eventId) {
    ...
    $.post(that.props.routes.DELETE_EVENT, { eventId: eventId }).then(function(
      response
    ) {
      if (response.success) {
        that.setState({
  ...
  addPlayer: function() {
    ...
    $.post(that.props.routes.SELECT_PLAYER, {
      ...
      if (response.success) {
        var player = that.state.players.filter(function(player) {
          return player.id == that.state.addPlayerId;
        });
        that.setState({
          squad: player.concat(that.state.squad).sort(function(a, b) {
  ...
  render: function() {
    ...
    var eventsClassName =
      "col-sm-9 col-sm-offset-2 col-xs-11 col-xs-offset-1 u-fade-in";
    if (this.state.loadingEvents) eventsClassName += " u-fade-in--hidden";
    var playersClassName =
      "col-sm-8 col-sm-offset-2 col-xs-10 col-xs-offset-1 u-fade-in";
