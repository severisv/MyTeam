...
module.exports = React.createClass({
  ...
  componentDidMount() {
    const that = this
    $.getJSON(that.routes.GET_FACEBOOK_IDS).then((response) => {
      that.setState({
  ...
  renderAddModule() {
    if (this.state.addUsingFacebook) { return <FacebookAdd addPlayer={this.addPlayer} existingIds={this.state.existingIds} /> }
    return (
      <EmailAdd addPlayer={this.addPlayer} validationMessage={this.state.validationMessage} />
    )
  },
})
