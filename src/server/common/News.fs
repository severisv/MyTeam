...
type Author = { UrlName: string; Name: string }
...
module Queries =
    let getArticle : GetArticle =
        fun db clubId name ->
            ...
            query {
                for article in db.Articles do
                    ...
                    select (
                        ...
                        article.ImageUrl,
                        article.Published,
                        (article.Author.FirstName, article.Author.MiddleName, article.Author.LastName),
            ...
            |> Seq.map
                (fun (name, headline, image, published, authorName, authorUrlName, content, gameId, hideAuthor, id) ->
                    { Id = id
                      Details =
                          { Name = name
                            Headline = headline
            ...
            (if not article.HideAuthor then
                 p [ _class "news-author" ] [
                     a [ _class "underline"
                         ...
                         _href
                         <| sprintf "/spillere/vis/%s" article.Author.UrlName ] [
                         encodedText article.Author.Name
                     ]
                     span [ _class "datetime" ] [
                         encodedText
                         <| sprintf " %s" (Date.format article.Details.Published)
                         whitespace
                         (if article.Details.Published.Year
                             <> DateTime.Now.Year then
                              encodedText
                              <| string article.Details.Published.Year
                          else
                              empty)
                     ]
