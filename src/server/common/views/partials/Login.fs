...
module Login =
    ...
    let loginPartial (ctx: HttpContext) =
        if ctx.User.Identity.IsAuthenticated then
            form [ _action
                   <| sprintf "/konto/utlogging?returnUrl=%s" (ctx.Request.Path + ctx.Request.QueryString)
                   _method "POST"
                   _id "logoutForm"
                   _class "navbar-topRight--item navbar-right signout-form" ] [
                antiforgeryToken ctx
                ul [ _class "nav navbar-nav navbar-right" ] [
                    li [] [
                        a [ _href "javascript:document.getElementById('logoutForm').submit()"
                            _title "Logg ut" ] [
