...
module.exports = React.createClass({
  ...
  renderPlayerLink(actions, event, model) {
    if (event.playerId) {
      const showPlayerUrl = `${model.showPlayerUrl}/${actions.getPlayerUrlName(event.playerId)}`
      return (
  ...
  renderAssist(event, actions, model) {
    if (event.assistedById) {
      const showPlayerUrl = `${model.showPlayerUrl}/${actions.getPlayerUrlName(event.assistedById)}`
      return (
        <span className="no-wrap">
  ...
  renderDeleteButton(event, actions, model) {
    if (model.editMode != false) {
      return (
        <a className="pull-right" onClick={actions.deleteEvent.bind(null, event.id)}>
