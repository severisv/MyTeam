    ...
    let index (club: Club) user selectedTeamShortName selectedYear (ctx: HttpContext) =
        ...
        |> function
            ...
            | Some selectedTeam ->
                ...
                let years =
                    query {
                        for game in db.Games do
                            where (
                                teamIds.Contains(game.TeamId.Value)
                ...
                let stats =
                    StatsQueries.get db selectedTeam selectedYear
                        ...
                        !!(navListMobile
                            { Items =
                              ...
                              Footer =
                                  Some
                                  <| { Text = "Total"
                                       Url = statsUrl selectedTeam AllYears }
                              IsSelected = isSelected })
                        hr []
                        br []
                        table
                            [ Striped
                              TableProperty.Attribute <| _class "stats-table" ]
