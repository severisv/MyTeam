...
let internal view model validationErrors (club: Club) (user: User option) (ctx: HttpContext) =
    user
    |> function
                       ...
                       | (false, _) ->
                           [ h4 [] [ encodedText "Velkommen!" ]
                             div [ _class "text-center" ] [
                                 p [] [
                                     encodedText
                                     <| sprintf "Her kan du be om spillertilgang til %s." club.ShortName
                                 ]
                                 p [] [
                                     encodedText "Først må du "

                                     a [ _href "konto/innlogging?returnUrl=/blimed" ] [
                       ...
                       | (true, Some model) ->
                           [ p [ _class "text-center" ] [
                             ...
                             !!(form [ Horizontal 2
                                 ...
                                 formRow [ Horizontal 2 ] [] [ btn [ Primary ] [ Helpers.str "Send" ] ]
                                ]) ]
                       | (true, None) ->
                           [ h4 [ _class "text-left" ] [
                               encodedText "Forespørselen din er mottatt"
                             ]
...
let get (club: Club) user (ctx: HttpContext) =
    ...
    ctx.Database.MemberRequests.Where
        (fun mr ->
            mr.ClubId = clubId
            && mr.Email = ctx.User.Identity.Name)
    |> Seq.tryHead
    |> function
        ...
        | None ->
            let model =
                { Fornavn =
                      ctx.User.GetClaim "facebookFirstName"
                      |> Option.defaultValue ""
                  Mellomnavn = ""
                  Etternavn =
                      ctx.User.GetClaim "facebookLastName"
                      |> Option.defaultValue ""
                  FacebookId =
                      ctx.User.GetClaim "facebookId"
                      |> Option.defaultValue "" }
        ...
        | Ok form ->
            ...
            |> function
                | Ok _ ->
                    ...
                    |> fun logger -> logger.LogInformation(sprintf "Ny medlemsforespørsel: \n%O" form)
