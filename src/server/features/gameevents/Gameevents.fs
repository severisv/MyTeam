...
let add: Add =
    let cardDoesNotHaveAssist _ (model: GameEvent) =
        if model.Type <> GameEventType.Mål
           && model.AssistedById.IsSome then
            Error "Kort kan ikke ha assist"
        else
            Ok()

    let isNotAssistedBySelf _ (model: GameEvent) =
        if model.PlayerId.IsSome
           && model.PlayerId = model.AssistedById then
    ...
    fun clubId gameId (ctx: HttpContext) (model: GameEventForm) ->
        let model: GameEvent =
            { Id = Guid.Empty
              ...
              AssistedById = model.AssistedById }
    ...
    fun clubId (gameId, gameEventId) (db: Database) ->
        ...
        |> function
            ...
            | Some gameEvent ->
                ...
                db.SaveChanges() |> ignore |> OkResult
