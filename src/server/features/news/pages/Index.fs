...
let private pagination articles =
    ...
    let previousUrl =
        sprintf "/%i/%i" (page.Skip - page.Take) page.Take
    ...
    div [ _class "mt-container" ] [
        nav [ _class "news-pager" ] [
            ul [ _class "pager" ] [
                li [ _class
                     <| (articles.HasPrevious =? ("", "disabled")) ] [
                    a [ (articles.HasPrevious
                         =? (_href previousUrl, _empty)) ] [
                        encodedText "← Nyere"
                    ]
                ]
                li [ _class <| (articles.HasNext =? ("", "disabled")) ] [
                    a [ (articles.HasNext =? (_href nextUrl, _empty)) ] [
                        encodedText "Eldre →"
                    ]
                ]
            ]
...
let view (club: Club) (user: User option) getPaginationOptions (ctx: HttpContext) =
    ...
    let paginationOptions =
        getPaginationOptions { Skip = 0; Take = 6 }

    let db = ctx.Database
    let description = Queries.getClubDescription db club.Id

    let articles =
        Queries.listArticles db club.Id paginationOptions
