...
let private editView
    ...
    let latestGames =
        Queries.listRecentGames db club.Id article.Id published

    [ mtMain [] [
        block [] [
            form [ _action (
                       name
                       |> Option.map (fun name -> sprintf "/nyheter/endre/%s" name)
                ...
                input [ _name "imageUrl"
                        _type "hidden"
                        _value article.ImageUrl ]
                input [ _name "id"
                        _type "hidden"
                        _value <| string article.Id ]

                div [ _class "form-file-upload-wrapper btn btn-default" ] [
                    input [ _name "file"
                            _type "file"
                            _class "cloudinary-fileupload pull-left"
                            attr "data-cloudinary-field" "imageUrl" ]
                ]
                Client.view
                     ...
                     |> List.collect
                         (fun error ->
                             ...
                             |> List.map
                                 (fun e ->
                                     li [] [
                                         encodedText <| sprintf "%s: %s" error.Name e
                                     ])))
                div [ _class "form-group news-matchreportSelect" ] [
                    label [ _class "col-xs-3 col-sm-2 no-padding control-label" ] [
                        encodedText "Kamprapport?"
                    ]
                    div [ _class "col-xs-9 col-sm-10 flex" ] [
                        div [] [
                            input [ _name "IsMatchReport"
                                    _type "checkbox"
                                    _class "form-control"
                        ...
                        div [ _class "flex-2" ] [
                            span [ _id "GameIdWrapper"
                                   article.IsMatchReport =? (_empty, _hidden) ] [
                                select
                                    [ _name "GameId"
                                      _class "form-control" ]
                                    ([ option [ _value "" ] [
                                           encodedText "- Velg kamp -"
                                       ] ]
                                     @ (latestGames
                                        |> List.map
                                            (fun game ->
                                                option [ Some game.Id = article.GameId
                                                         =? (_selected, _empty)
                                                         _value <| string game.Id ] [
                                                    encodedText game.Name
                                                ])))
                            ]
                        ]
                    ]
                ...
                div [ _class "form-group" ] [
                    label [ _class "col-xs-3 col-sm-2 no-padding control-label"
                            _style "margin-top: 0.5em;" ] [
                        encodedText "Skjul forfatter"
                    ]
                    div [ _class "col-xs-9 col-sm-10 flex" ] [
                        div [] [
                            input [ _name "HideAuthor"
                                    _type "checkbox"
                                    _class "form-control"
                ...
                br []
                div [] [
                    label [ _name "Headline" ] [
                        encodedText "Overskrift:"
                    ]
                    div [ _class "form-group" ] [
                        input [ _class "form-control"
                                _name "Headline"
                                _placeholder "Sensasjonell snuoperasjon av Sleivdal"
                                _value article.Headline ]
                    ]
                ]
                div [ _class "form-group" ] [
                    textarea [ _name "Content"
                               _class "form-control update-table tinymce"
    ...
    |> layout
        ...
        (fun o ->
            { o with
                  Title =
                      name
                      |> Option.map (fun _ -> "Rediger artikkel")
                      |> Option.defaultValue "Skriv ny artikkel"
                  Scripts =
                      Components.tinyMceScripts
                      @ Images.uploadScripts cloudinarySettings.Value })
        ctx
    |> OkResult
...
let editPost (club: Club) (user: User) name (ctx: HttpContext) =
    ...
    combine [ <@ form.Headline @> >- [ isRequired ]
              <@ form.Content @> >- [ isRequired ] ]
    |> function
        | Ok () ->
            Persistence.saveArticle db club.Id name form
            |> Results.bind (fun _ -> Redirect <| sprintf "/nyheter/vis/%s" name)
        ...
        | Error validationErrors ->
            ...
            |> function
                ...
                | Some article ->
                    ...
                    |> fun (article, published) -> editView ctx club user (Some name) article published validationErrors
...
let create (club: Club) user (ctx: HttpContext) =
    ({ Id = Guid.Empty
       ...
       HideAuthor = false },
     System.DateTime.Now)
    |> fun (article, published) -> editView ctx club user None article published []
...
let createPost (club: Club) (user: User) (ctx: HttpContext) =
    ...
    |> function
        | Ok () ->
            ...
            |> Results.bind (fun name -> Redirect <| sprintf "/nyheter/vis/%s" name)


        | Error validationErrors ->
            (form, System.DateTime.Now)
            |> fun (article, published) -> editView ctx club user None article published validationErrors
