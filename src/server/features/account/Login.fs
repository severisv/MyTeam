...
let internal addClaim (info: ExternalLoginInfo) (userManager: UserManager<ApplicationUser>) (user:ApplicationUser) name claimType =
            ...
            task {
                ...
                if isNull info then       
                    ...
                    | true ->
                        ...
                        logger.LogInformation(EventId(3), (sprintf "User %s created a new account with Facebook." form.``E-post``))
                                          
                        let facebookId =
                           match info.Principal.Claims |> Seq.tryFind (fun c -> c.Type = ClaimTypes.NameIdentifier) with
                           | Some facebookId -> facebookId.Value
                           | None -> failwithf "Could't find facebookId when creating account from external. %O" form.``E-post``
                        
                        let db = ctx.Database                   
                        let player = db.Members.FirstOrDefault(fun p -> p.FacebookId = facebookId)
