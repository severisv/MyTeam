...
let view (club: Club) (user: User) (year: string option) (ctx: HttpContext) =
    ...
    let allFines =
        ...
        |> List.map
            (fun ((memberId, year), values) ->
                { MemberId = memberId
                  Year = year
                  Amount =
                      values
                      |> List.sumBy (fun (_, amount, _) -> amount) })
    ...
    let sum (year: SelectedYear) memberId =
        let filterBy =
            function
            | Year year -> List.filter (fun a -> a.Year <= year && a.MemberId = memberId)
            | AllYears -> List.filter (fun a -> a.MemberId = memberId)

        filterBy year >> List.sumBy (fun a -> a.Amount)

    let fines =
        let memberIds =
            allFines |> List.map (fun a -> a.MemberId)
    ...
    let remainingForUser =
        Some
        <| (allFines |> sum AllYears user.Id)
           - (payments |> sum AllYears user.Id)
        |> Option.bind (fun a -> if a > 0 then Some a else None)

    let createUrl =
        function
        | AllYears -> "/intern/boter/oversikt/total"
        | Year y -> sprintf "/intern/boter/oversikt/%i" y


    let isSelected url = createUrl year = url
    ...
    [ mtMain [] [
        ...
        block [] [
            ...
            => fun amount ->
                div [ _class "fine-userDue" ] [
                    ...
                    div [] [
                        ...
                        encodedText (
                            paymentInformation
                            |> Option.defaultValue "Det er ikke lagt til noen betalingsinformasjon."
                        )
                    ]
                ]
            table
                ...
                [ col [ CellType Image; NoSort ] []
                  ...
                  col [ NoSort; Align Center ] [
                      !!(Icons.fine "Utestående")
                      whitespace
                      encodedText "Utestående"
                  ]
                  col [ NoSort; Align Right ] [
                      !!(Icons.fine "Utestående")
                      whitespace
                      encodedText "Total"
                  ] ]
                 ...
                 |> List.map
                     (fun { Member = player
                         ...
                         let playerLink =
                             ...
                             a [ _href
                                 <| sprintf "/intern/boter/vis/%s/%O" year player.Id ]
            ...
            div [ _class "fine-total" ] [
                ...
                b [] [
                    encodedText
                    <| sprintf "%i,-" (fines |> List.sumBy (fun f -> f.Total))
                ]
            ]
        ]
      ]
