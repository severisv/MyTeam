...
let save setState gameId (lineups: GamePlanState) =
    ...
    promise {
        ...
        setState (fun state props -> { state with ErrorMessage = None })
    }
    |> Promise.catch (fun e ->
        Browser.Dom.console.error (sprintf "%O" e)
        setState (fun state props -> { state with ErrorMessage = Some "Tilkoblingsproblemer, endringene blir ikke lagret" }))
    |> Promise.start
...
let getName (players: Members.Member list) (player: Members.Member) =
    let rec getName numberOfLettersFromLastName =
        ...
        match (players
               |> List.filter (fun p -> hashName p = name))
            .Length
            with
        | i when i < 2 -> name
        | _ when
            numberOfLettersFromLastName
...
let handleTimeChange setState save lineupId value =
    updateLineup setState save lineupId (fun line -> { line with Time = value })

let handlePlayerChange setState save lineupId index value =
    updateLineup setState save lineupId (fun line ->
        { line with
            Players =
                line.Players
                |> List.mapi (fun i playerName -> if i = index then value else playerName) })

let duplicateLineup setState save lineupId =
    setState (fun state props ->
        let line =
            state.GamePlan.Lineups
            |> List.find (fun line -> line.Id = lineupId)
...
type GamePlan(props) =
    ...
    override this.render() =
        ...
        let subs lineup =
            ...
            |> List.filter (fun p ->
                lineup
                |> List.exists (fun l -> l = (Some <| p.FirstName))
                |> not)
        ...
        let square (lineup: List<string option>) lineupId row col =
            ...
            |> function
                | Some playerIndex ->
                    ...
                    let getPlayer name =
                        state.Players
                        |> List.tryFind (fun p -> p.FirstName = name)

                    let playerImage (name: string) =
                        img [
                            Class "gameplan-playerImage"
                            Src(
                                Image.getMember model.ImageOptions (fun o ->
                                    { o with
                                        Height = Some 40
                                        Width = Some 40 })
                                |> fun img ->
                    ...
                    fragment [] [
                        ...
                        input [
                            Type "text"
                            Class(
                                if state.FocusedPlayer = Some(lineupId, playerIndex) then
                                    "focused"
                                else
                                    ""
                            )
        ...
        fragment [] [
            ...
            mtMain [ Class "gameplan"
                     OnClick(fun _ -> this.setState (fun state props -> { state with FocusedPlayer = None })) ] [
                block
                    []
                    ([ (state.ErrorMessage
                        |> function
                                ...
                                div [ Key <| string lineup.Id ] [
                                    div [ Class "text-center" ] [
                                        input [
                                            ...
                                            Value lineup.Time
                                            OnChange (fun e ->
                                                handleTimeChange lineup.Id
                                                <| (Number.tryParse e.Value |> Option.defaultValue 0))
                                            OnFocus handleFocus
                                            OnBlur sortLineup
                                        ]
                                        str "min"
                                    ...
                                    div
                                        ...
                                        (state.GamePlan.Lineups
                                         ...
                                         |> fun i ->
                                             if i > 0 then
                                                 Some state.GamePlan.Lineups.[i - 1]
                                             else
                                                 None
                                         |> function
                                             | Some prevLineup ->
                                                 let tryGet (list: string list) i =
                                                     if list.Length > i then list.[i] else ""

                                                 let ins =
                                                     lineup.Players
                                                     |> List.except prevLineup.Players
                                                     |> List.choose id

                                                 let outs =
                                                     prevLineup.Players
                                                     |> List.except lineup.Players
                                                     |> List.choose id

                                                 [ 0 .. (max ins.Length outs.Length) - 1 ]
                                                 |> List.map (fun i ->
                                                     div [ Class "text-center gp-subs" ] [
                                                         span [ Class "gameplan-sub-in" ] [
                                                             str <| tryGet ins i
                                                         ]
                                                         str " -> "
                                                         span [ Class "gameplan-sub-out" ] [
                                                             str <| tryGet outs i
                                                         ]
                                                     ])
