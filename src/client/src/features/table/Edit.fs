...
open Client.Components.AutoSync
                                  ...
                                  Content =
                                    fun handleClose ->
                                        div [] [
                                            h4 [] [
                                                str
                                                <| sprintf "Er du sikker på at du vil slette tabellen for %s %i?" props.Team props.Year
                                            ]
                                            div [ Class "text-center" ] [
                                                ...
                                                Send.sendElement (fun o ->
                                                    { o with
                                                        ...
                                                        Endpoint =
                                                            Send.Delete
                                                            <| sprintf "/api/tables/%s/%i" props.Team props.Year
                                ...
                                &? formRow [ str "Url til kamper på fotball.no" ] [
                                    Text.Element
                                        { Value = state.FixtureSourceUrl
                                          ...
                                          OnChange =
                                            Some
                                            <| fun value -> update (fun state -> { state with FixtureSourceUrl = value }) }
                                   ]
                                br []
                            ]
                           ]
