...
type State = { Payments: Payment list }
...
let element props children =
    komponent<PaymentsModel, State> props { Payments = props.Payments } None (fun (props, state, setState) ->
        let year = props.Year
                        ...
                        (fines
                         |> List.map (fun payment ->
                             let playerLink =
                                 a [
                                     Href
                                     <| createUrl props.Year (Member payment.Member.Id)
                                 ]
                                         ...
                                         <| Image.getMember
                                             ...
                                             (fun o ->
                                                 { o with
                                                     Height = Some 50
                                                     Width = Some 50 })
                                             payment.Member.Image
                                             payment.Member.FacebookId
                                     ]
                                 ]
                                 fragment [] [
                                     playerLink [ str payment.Member.Name ]
                                         ...
                                         fun handleClose ->
                                             div [] [
                                                 h4 [] [
                                                     str
                                                     <| sprintf
                                                         "Er du sikker på at du vil slette %i,- fra %s (%s) ?"
                                                         payment.Amount
                                                     ...
                                                     Send.sendElement (fun o ->
                                                         { o with
                                                             ...
                                                             OnSubmit =
                                                                 Some(
                                                                     !>handleClose
                                                                     >> (fun _ -> handleDeleted setState payment.Id)
                                                                 ) })
            ...
            sidebar [] [
                ...
                &? block [] [
                    ...
                    AutoSync.TextArea.render
                        { Value = props.PaymentInformation
