...
let loadMoreEvents setState _ =
    setState (fun state props -> { state with FetchMoreState = Fetching })
                        ...
                        [ div [ Class "row" ]
                                    ...
                                    [ tabs []
                                          [ { Text = " Kommende"
                                            ...
                                            { Text = " Tidligere"
                                              ...
                                              Icon = Some(Icons.previous "") } ] (fun url ->
                                          match props.Period with
                          ...
                          fr []
                              (state.Events
                               |> List.map (fun event ->
                                   ...
                                   let signups =
                                       state.Signups.TryFind(event.Id)
                                       |> Option.defaultValue []

                                   let attending =
                                       signups |> List.filter (fun ea -> ea.IsAttending)

                                   let notAttending =
                                       signups
                                       |> List.filter (fun ea -> not ea.IsAttending)
                                                       ...
                                                       | (Game game, _) when game.GamePlanIsPublished ->
                                                           [ a
                                                               [ Class "edit-link pull-right"
                                                                 ...
                                                                 [ Icons.gamePlan ] ]
                                                       | (Game game, _) -> []
                                                       | (Training, Previous _) when user.IsInRole
                                                                                         [ Admin; Trener; Oppmøte ] ->
                                                           [ a
                                                               [ Class "edit-link pull-right"
                                                                 Href
                                                      ...
                                                      div [ Class "event-signup-listplayers" ]
                                                          [ Collapsible.collapsible Collapsed
                                                                [ span [ Class "flex-2" ]
                                                                  ...
                                                                  span [ Class "flex-2" ]
                                                                      [ str
                                                                        <| sprintf "Kan ikke (%i)" notAttending.Length ]
                          ...
                          &? div [ Class "event-showAll" ]
                                 [ (match state.FetchMoreState with
                                    | Unfetched ->
                                        a [ OnClick <| loadMoreEvents setState ] [ Icons.addCircle ""; str " Vis alle" ]
                                    | Fetching -> Icons.spinner
                                    | Error e ->
                                        fr []
                                            [ div [ Class "red" ] [ Icons.warning; whitespace; str e ]
                                              a [ OnClick <| loadMoreEvents setState ] [ str "Prøv på nytt" ] ]
                                    | Success -> empty) ] ] ]
              sidebar []
