...
module Pages =
    ...
    let internal coachMenuItems = [
                                    ...
                                    |> List.map (fun (href, icon, text) -> li [] [a[_href href][ !!icon; encodedText text ]] )
    ...
    let internal internalMenuItems =
        ...
        |> List.map (fun (href, icon, text) -> li [] [a[_href href][ !!icon; encodedText <| sprintf " %s" text ]] )
    ...
    let layout club (user: Option<User>) getOptions (ctx: HttpContext) content =
        ...
        let assetHashes =
            ctx.GetService<IOptions<AssetHashes>>().Value
                                     ...
                                     ([ ul [ _class "nav navbar-nav" ]
                                            [ li [] [ a [ _href "/spillere" ] [ encodedText "Spillere" ] ]
                                              ...
                                              |> Option.fold (fun _ _ ->
                                                  li []
                                                      [ a
                                                          [ _class "slide-down-parent hidden-xs"
                                              ...
                                              =? (li []
                                                      [ a [ _href "/admin" ]
                                                            [ !!(Icons.coach "")
                                                              encodedText " Admin" ] ],
                                                  emptyText) ])) emptyText ] ]
                     div [ _class "mt-page-header" ] [ div [ _class "container" ] [ h1 [] [ encodedText o.Title ] ] ]
                     div
                         [ _id "main-container"
                           _class "container" ] content
